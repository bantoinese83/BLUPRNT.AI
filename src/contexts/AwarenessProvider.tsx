...
export function AwarenessProvider({
  ...
  const awarenessData = useMemo(() => {
    if (!project) return { insights: [], projectHealth: 'optimal' as const, nextBestAction: null };
    ...
    invoices.forEach(inv => {
      // @ts-ignore: category added via migration
    ...
    if (project.stage === 'planning' && scopeItems.length === 0) {
      newInsights.push({
        ...
        description: 'Your project is in the planning stage. Adding scope items will help you track costs more accurately.',
    ...
    if (project.stage === 'construction' && invoices.length === 0) {
      newInsights.push({
        ...
        description: "You're in the construction phase but haven't uploaded any invoices yet. Start tracking to stay on budget.",
    ...
    return { insights: newInsights, projectHealth: health, nextBestAction: nextAction };
  }, [project, scopeItems, invoices]);

  useEffect(() => {
    setInsights(awarenessData.insights);
    setLoading(false);
  }, [awarenessData]);

  return (
    <AwarenessContext.Provider value={{ 
      ...awarenessData, 
      loading, 
      isSidebarOpen, 
      setIsSidebarOpen 
