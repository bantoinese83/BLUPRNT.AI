...
export function PropertyLedger({
  ...
  async function handleExportPDF() {
    ...
    setMessage(null);
    try {
      const blob = generateSellerPacketBlob(project, scopeItems, invoices);
      ...
      setMessage("We couldn’t finish that. Check your connection and try again.");
      onUpgradeClick?.();
    } finally {
      setExporting(false);
    }
      ...
      <Card id="property-ledger-anchor" className="glass-card border-white/60 shadow-xl shadow-slate-200/40 rounded-[2.5rem] overflow-hidden group">
        ...
        <div className="pt-2">
          ...
          >
            ...
            <span className="whitespace-nowrap">{exporting ? "Generating Packet…" : "Export Seller Packet"}</span>
          </Button>
          {message && (
            <p className="mt-4 text-[10px] text-center font-bold text-slate-400 uppercase tracking-widest">{message}</p>
          )}
        </div>
      </CardContent>
    </Card>
