...
test.describe("Billing Management", () => {
  ...
  test("Manage Plan button is visible on the settings page for Free tier", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole("button", { name: "Upgrade Now" }),
    ).toBeVisible();
    await expect(page.getByText("Free Explorer")).toBeVisible();
  });

  test("Manage Plan button shows loading state and calls portal function", async ({
    page,
  }) => {
