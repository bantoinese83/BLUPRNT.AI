  ...
  try {
    ...
    try {
      ...
      const parsed = photoToScopeSchema.safeParse({
        ...
        project_id:
          (formData.get("project_id") as string | null)?.trim() || null,
      ...
      const fileList = Array.from(photos).filter(
        (p): p is File => p instanceof File && p.size > 0,
      );
      if (fileList.length > MAX_VISION_PHOTOS) {
        console.warn(
          `[photo-to-scope] Using first ${MAX_VISION_PHOTOS} of ${fileList.length} photos (limits latency).`,
        );
      }
      const photoParts: GeminiPart[] = [];
      for (const p of fileList.slice(0, MAX_VISION_PHOTOS)) {
    ...
    const err = top instanceof Error ? top : new Error(String(top));
    console.error("[photo-to-scope] top-level:", err.stack ?? err);
    return jsonResponse(
      { error: "Something went wrong. Try again in a moment." },
      500,
      req,
